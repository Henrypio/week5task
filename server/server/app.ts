...
// console.log(dbPath);
// const database =
//   "C:/Users/okoli/Documents/week-5-task-node-sq19-Henrypio/server/database.json";
  ...
  (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === "GET") {
      ...
      if (!query.id) {
        ...
        const dataToWriteBack: databaseOBject[] = existingArray.filter(
        ...
        req.on("end", () => {
          fs.readFile(database, "utf-8", (err, dataReadFromFile) => {
            if (err) {
              ...
              fs.writeFileSync(
                database,
                JSON.stringify(existingArrayOfData, null, 2)
              );
              res.writeHead(200, { "content-type": "text/json" });
